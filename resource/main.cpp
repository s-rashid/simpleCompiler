...
void deserialTest(){
    ...
    QString source = "dci a\n"
                     "rdi a\n"
                     "LB: prt a\n"
                     "jmp LB\n"
                     "end";

    qDebug() << "compiling program";
    QString name = "deserialTEST123";
    control.compileProgram(nullptr, source, name);

    qDebug() << "deserialize json into new program";
    QString name_new = "deserialTEST_new123";
    Program* deserial = new Program(name_new);
    RunProgramControl rpc;
    rpc.deserialize(deserial, name);

    qDebug() << "serialize new program to compare structure";
    CompilerProgramControl cpc;
    cpc.serialize(deserial, name_new);

    //if deserialTEST.json == deserialTEST_new.json
}
void controlTest() {
